    ...
    <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 py-6">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-multirao-roxo">
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div class="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-sm">
                        ...
                        <div class="space-y-3">
                            ...
                            <p class="text-sm"><span class="font-bold text-gray-700 uppercase text-xs block">Série / Período (Escola):</span> {{ $crianca->serie }} - {{ $crianca->periodo_escolar }}</p>
                            <p class="text-sm"><span class="font-bold text-multirao-roxo uppercase text-xs block">Período Desejado (ONG):</span> <span class="font-black">{{ $crianca->periodo_ong ?? 'Não informado' }}</span></p>
                        </div>
                    </div>
                ...
                <form action="{{ route('triagem.status', $crianca->id) }}" method="POST">
                    <div class="bg-multirao-roxo/5 p-6 rounded-lg border-2 border-dashed border-multirao-roxo/20">
                        <h3 class="text-lg font-bold text-multirao-roxo mb-4 uppercase">Ação de Triagem</h3>
                    
                        @csrf
                        <div class="mb-4">
                            <label for="detalhes" class="block text-sm font-bold text-multirao-roxo uppercase mb-1">Observações da Triagem (Obrigatório em caso de Rejeição)</label>
                            <textarea name="detalhes" id="detalhes" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-multirao-roxo focus:ring-multirao-roxo" placeholder="Descreva aqui o motivo da aprovação ou rejeição..."></textarea>
                        </div>
                    </div>
                    <div class="flex gap-4 mt-4">
                        <button type="submit" name="status" value="PENDENTE_MATRICULA" onclick="return confirm('Confirmar aprovação desta triagem? A criança será movida para a fase de Matrícula.')" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow-md transition duration-300 uppercase tracking-wider">
                            Aprovar para Matrícula
                        </button>
                        <button type="submit" name="status" value="REJEITADO" onclick="return confirm('Tem certeza que deseja rejeitar esta inscrição?')" class="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg shadow-md transition duration-300 uppercase tracking-wider">
                            Rejeitar Inscrição
                        </button>
                    </div>
                </form>
