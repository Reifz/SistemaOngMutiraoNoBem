...
## [10/06/2026] - Flexibilização da Edição de Matrículas e Anamneses Concluídas
- **O que foi feito:**
    1. **Matrícula:**
        - Atualizada a lógica no `MatriculaController@formulario` para redirecionar automaticamente para a tela de edição caso a matrícula já tenha sido preenchida (status diferente de pendente).
        - Modificadas as views `matricula/index.blade.php` e `matricula/show.blade.php` para exibir o botão de "Editar" em todos os status ativos (Aprovada, Anamnese Concluída, Em Turma, etc), ocultando apenas para crianças evadidas.
    2. **Anamnese:**
        - Atualizadas as views `anamnese/index.blade.php` e `anamnese/show.blade.php` para permitir a edição dos dados mesmo após a conclusão ou alocação em turma.
    3. **Integridade de Dados:** Garantido que a edição manual não reverta automaticamente o status do aluno (ex: um aluno "Em Turma" continua "Em Turma" após a edição dos dados cadastrais).
- **Motivo:** Atendimento à solicitação de permitir correções e atualizações em prontuários já finalizados sem a necessidade de resetar o fluxo do aluno.

## [10/06/2026] - Expansão de Veteranos para Teste de Rematrícula
- **O que foi feito:**
    1. **Adição de Exemplos:** Incluídos mais 2 veteranos de 2025 (`Luiza Meirelles` e `Rodrigo Ferreira dos Santos`) no `TesteCompletoSeeder` que ainda não iniciaram a rematrícula para 2026.
- **Motivo:** Atendimento à solicitação do usuário para ter mais exemplos práticos ao testar o fluxo de renovação anual.

## [10/06/2026] - Geração de Massa de Dados para Testes (Plano de Testes - v2)
- **O que foi feito:**
    1. **Expansão de Dados:** Criados 20 registros de crianças com nomes reais e 100% dos campos preenchidos.
    2. **Variedade de Evasão:** Implementados cenários reais de evasão para ambos os turnos (Manhã e Tarde) com motivos diversificados (Mudança, Inadaptação, etc).
    3. **Rematrícula (V2):** Adicionados casos de veteranos de 2025 em diferentes estágios (Não iniciado, Pendente Matrícula, Pendente Anamnese e Rematriculado).
    4. **Bug Fix:** Corrigida falha no `RematriculaController` que exibia crianças já evadidas na lista de renovação anual.
    5. **Caso Complexo:** Inserção do registro "Manuel Ferraz" com família de 5+ integrantes e histórico de logs robusto para teste de performance de listagem.
- **Motivo:** Homologação final do sistema e validação da lógica de virada de ano e rematrícula.

## [10/06/2026] - Geração de Massa de Dados para Testes (Plano de Testes) [OBSOLETO]
...
## [25/06/2026] - Implementacao das Melhorias v2
- **O que foi feito:**
    1. **Pre-inscricao publica:** destaque nas informacoes importantes, aumento de fonte, contra-turno automatico e bloqueio administrativo de inscricoes por configuracao JSON local.
    2. **Painel administrativo:** criada tela de Configuracoes para administradores ativarem/desativarem inscricoes publicas.
    3. **Dashboard:** alerta de triagem parada reduzido de 7 para 5 dias.
    4. **Matricula:** uploads de documentos passam a aceitar PDF e imagens/fotos; adicionada opcao "Dificuldade de deslocamento" na evasao.
    5. **Anamnese:** caracteristicas da crianca migradas para multiselecao com compatibilidade para dados antigos em texto.
    6. **PDFs:** borda inferior do cabecalho reforcada para 4px.
    7. **Turmas:** Manha identificada em amarelo e Tarde em azul em listagens, prontuario da turma e badges.
    8. **Historico:** filtro por periodo de datas no prontuario.
    9. **Desistencia:** registro disponivel em triagem, matricula e anamnese, usando `data_desligamento`, `motivo_desligamento` e status `DESISTENTE`.
- **Motivo:** Conclusao do plano `plano_de_acao_melhorias.md`.

## [03/07/2026] - Ajustes de Robustez e Acessibilidade (Melhorias v2)
- **O que foi feito:**
    1. **ConfiguracaoService:** Adicionada validação de array após `json_decode` nos métodos `get` e `set` para evitar erros caso o arquivo JSON de configurações esteja vazio ou inválido.
    2. **Pré-Inscrição:** Ajustado o tamanho da fonte (`font-size`) dos campos de entrada (inputs e selects) de `16px` para `1.05rem` para harmonizar com a fonte base de acessibilidade da página (19px).
- **Motivo:** Aplicação das melhorias recomendadas na análise do plano de melhorias v2.
- **Resultado:** Aumentada a robustez contra erros de arquivo local e melhorada a acessibilidade nos inputs públicos.

## [03/07/2026] - Correcoes das Observacoes `obs dev` (Melhorias v2)
- **O que foi feito:**
    1. **Pre-inscricao publica:** Ajustado o bloco de informacoes importantes para usar uma unica cor, com texto preto e fundo vermelho sutil, alem de melhorias de responsividade mobile.
    2. **Dashboard:** Corrigido o texto do card de alertas de triagem para exibir 5 dias.
    3. **PDFs:** Cabecalhos reforcados para borda inferior de 8px.
    4. **Turmas:** Cards da listagem receberam lateral com cor inline mais forte e badges de turno com amarelo/azul mais marcados.
    5. **Menu mobile:** Hamburger passou a exibir os mesmos atalhos principais do menu desktop, incluindo itens administrativos.
- **Motivo:** Aplicacao das observacoes marcadas como `obs dev` em `plano_de_acao_melhorias.md`.
- **Resultado:** Ajustes visuais e de navegacao alinhados ao feedback de validacao.
